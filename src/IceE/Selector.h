...
//        _pollFdSet.push_back(pollFd);
    ...
    int select()
    {
        while(true)
        {
            ...
            if(_timeout > 0)
            {
                ...
                tv.tv_sec = _timeout;
                tv.tv_usec = 0;
                ret = ::select(nFd, rFdSet, wFdSet, eFdSet, &tv); // The first parameter is ignored on Windows
            }
            else
            {
            	ret = ::select(nFd, rFdSet, wFdSet, eFdSet, 0); // The first parameter is ignored on Windows
            }
#else
    ...
    {
        int nCount1 = IceUtil::getFdsetBits(_selectedReadFdSet.fds_bits[j]);
    ...
    int nCount3 = IceUtil::getFdsetBits(_selectedErrorFdSet.fds_bits[j]);
...
private:
    ...
    void addImpl(T* handler, SocketStatus status)
    {
        ...
        _handlerMap.insert(std::make_pair(handler->_fd, handler));
#if defined(ICE_USE_SELECT)
        switch(status)
        {
        case NeedRead:
   ...
   //{
    ...
    fdSetCopy(fd_set& dest, fd_set& src)
   {
    	  for (int j = 0; j < howmany(FD_SETSIZE, NFDBITS);++j)
    	  {
    		  dest.fds_bits[j] = src.fds_bits[j];
    		  return &dest;
    	  }
        return 0;
    }
